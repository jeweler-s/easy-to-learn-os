...
### ✅ 10) 교착 상태와 아사 현상의 차이 (296pg)

- 아사 현상 : 잘못된 정책으로 특정 프로세스의 작업이 지연되는 문제
- 교착 상태 : 여러 프로세스가 작업을 진행하다 발생하는 자연적인 현상

### ✅ 11) 교착 상태 필요 조건 (300pg)

- 상호 배제 : 한 프로세스가 사용하는 자원은 다른 프로세스와 공유할 수 없는 배타적인 자원이어야 함
- 비선점 : 한 프로세스가 사용중인 자원은 중간에 다른 프로세스가 빼앗을 수 없음
- 점유와 대기 : 프로세스가 어떤 자원을 할당받은 상태에서 다른 자원을 기다리는 상태여야 함
- 원형 대기 : 점유와 대기를 하는 프로세스 간의 관계가 원을 이루어야 함

### ✅ 12) 교착 상태 해결 방법의 종류 (303pg)

- 예방 : 교착 상태를 유발하는 네 가지 조건이 발생하지 않도록 무력화
- 회피 : 자원 할당량을 조절하여 교착 상태 해결
- 검출 : 자원 할당 그래프를 모니터링 하며 교착 상태가 발생하는지 확인
- 회복 : 교착 상태를 유발한 프로세스를 강제 종료
